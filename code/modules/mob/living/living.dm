...
/mob/living/proc/do_damage_animation(damage, target_zone="chest", i_state="brute")
	var/scale = Clamp(damage/25, 0.25, 0.8)
	var/image/I = image('icons/effects/onhit.dmi', src, i_state, src.layer + 1)
	...
	I.alpha = 225
	animate(I, transform = I.transform * 1.2, alpha = 255, time = 4)
	spawn(4)
		I.icon_state = "[I.icon_state]_done" //Doing animate() on animated sprites will reset their animations on next usage of the proc.
	animate(transform = I.transform * 1.3, alpha = 0, time = 3)
